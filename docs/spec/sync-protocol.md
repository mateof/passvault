...
The device must have registered its signing key first, with `POST /api/v1/devices`.
The server verifies every operation against a registered key and quarantines what it
cannot verify, so an unregistered device uploads a wallet that lands entirely in
quarantine — and is told it succeeded.

#### Publishing an event the server has never seen

An event created offline is uploaded by synchronising it: no separate call, and no
identifier minted by the server. When `{eventId}` names nothing here and the batch
carries an `event.create` scoped to it, the server creates the event under **that**
identifier — a new one would orphan every operation the device has already signed.

Three conditions, and each one is why a different attack does not work: the batch
declares the event; the device that signed the declaration is registered, active and
belongs to the caller; and the signature verifies. Failing any of them leaves the
event absent, and the push then fails as it would for any unknown event.

The event key is generated by the server, not uploaded. Operations carry their fields
under the transport's encryption and the server re-encrypts them into its own columns,
so a key from the device would unlock nothing here. Sending `eventPassword` on the
call that creates it produces a password-protected event, with no server slot in the
envelope — the same property, and the same cost, as creating one through the API.

The response says `created: true` on the call that brought the event into existence,
so a client can distinguish "your event is now on the server" from "we exchanged some
operations".
