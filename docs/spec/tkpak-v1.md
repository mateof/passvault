...
`npm run vectors:generate`, each a JSON descriptor naming a `.tkpak`, the password
or private key that opens it, and the expected decrypted result:

| Vector | Exercises |
| --- | --- |
| `01-single-ticket-password` | Minimal file, one `argon2id` slot, no blobs. |
| `02-multi-ticket-pdf-blobs` | Four tickets, three PDF page blobs. |
| `03-sealed-to-recipient` | Only an `x25519-sealed` slot. |
| `04-dual-slot` | Password and sealed slot over one file key. |
| `05-pkpass-blob` | An Apple Wallet pass as a blob. |
| `06-tampered-payload` | One ciphertext byte flipped. Must yield `DECRYPTION_FAILED`. |
| `07-tampered-manifest` | Preview edited after signing. Must yield `BAD_SIGNATURE`. |
| `08-wrong-version` | `version: 2`. Must yield `UNSUPPORTED_VERSION`. |
| `09-unicode-password` | Password with combining accents, tests NFC normalisation. |
| `10-minimal-preview` | `preview` with `ticketCount` only. |
...
the file says, which is what makes both work.
